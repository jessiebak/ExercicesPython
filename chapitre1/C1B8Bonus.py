...
def ShowResult(_prixHT,_reduc):
    ...
    _prixReduc =  Tax(Conversion(_prixHT,_dollars),_dollartax) - Conversion(_reduc, _dollars)
    ...
    print (f"Prix en dollars TC : {_prixTTC}")
    print(f"Prix total avec réduction : {_prixReduc}")

    _prixConv = Conversion(_prixHT,_mn)
    _prixTTC= Tax(_prixConv,_mntax)
    _prixReduc =  Tax(Conversion(_prixHT,_mn),_mntax) - (Conversion(_reduc,_mn))
